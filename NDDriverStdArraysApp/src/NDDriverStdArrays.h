...
private:
    ...
    template <typename epicsType, typename NDArrayType> void copyBuffer(size_t nextElement, size_t stride, void *pValue, size_t nElements);
    template <typename NDArrayType> void fillBuffer(double fillValueDouble);
    void doCallbacks();
    void setArrayComplete();
    size_t arrayDimensions_[ND_ARRAY_MAX_DIMS];
    NDArrayInfo arrayInfo_;
    size_t dimProd_[ND_ARRAY_MAX_DIMS];
};
